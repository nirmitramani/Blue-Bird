...
const CouponCode = () => {
  ...
  const handleReorder = async (newOrder) => {
    try {
      await axios.post(`${window.react_app_url + window.coupon_code_url}/reorder`, { newOrder });
      return true;
    } catch (error) {
  ...
  useEffect(() => {
    ...
    couponCodes.forEach((couponCode) => {
      initialSwitchStates[couponCode._id] = couponCode.status === 'Active';
    });
  ...
  useEffect(() => {
    ...
    axios.get(`${window.react_app_url + window.coupon_code_url}`)
      .then(result => {
        setCouponCodes(result.data.data);
        stopAnimatedLoading()
      })
      .catch(err => {
        stopAnimatedLoading()
  ...
  const handleStatusChange = (id) => {
    ...
    axios.put(`${window.react_app_url + window.coupon_code_url}/status/${id}`, { status: newStatus })
      .then(res => {
        console.log(res);
  ...
  const handleDelete = (id) => {
    ConfirmDelete()({
      ...
      onConfirm: () => deleteCoupneCode(id),
    });
  };

  const deleteCoupneCode = async (id) => {
    try {
      const response = await axios.delete(`${window.react_app_url + window.coupon_code_url}/${id}`);
      setCouponCodes((prevcouponCode) => prevcouponCode.filter((couponCode) => couponCode._id !== id));
  ...
  const filteredcouponcodes = useMemo(() => {
    return couponCodes.filter((couponCode) => {
      return couponCode.title.toLowerCase().includes(searchQuery.toLowerCase()) || couponCode.description.toLowerCase().includes(searchQuery.toLowerCase());
    });
  ...
  return (
    <>
      ...
      <div className="p-2 flex flex-col space-y-6 md:space-y-0 md:flex-row justify-between">
        <BreadCrumb title="Coupon Code" desc="" link="/admin" />
        <div className="flex flex-wrap items-start justify-end -mb-3 p-4">
          <Link to="/admin-coupon-code-add">
            <Button label="Add new Coupon Code" iconURL={<AiOutlinePlus />} />
          </Link>
        </div>
      </div>
                  ...
                  <table className="min-w-full leading-normal">
                    <thead>
                      <tr>
                        ...
                        <th className="px-5 py-3 w-64 border-b-2 border-gray-200 bg-slate-200 text-left text-xs font-semibold text-gray-700 uppercase tracking-wider">
                          ACTIONS
                        </th>
                      </tr>
                    </thead>
                    <tbody>
                      {currentItems.map((couponCode, index) => (
                        ...
                        >
                          ...
                          <td className="px-3 py-3 w-64 border-b border-gray-200 bg-white text-sm space-x-2">
